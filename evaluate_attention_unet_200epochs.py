...
from train_attention_unet_200epochs import calculate_metrics

# --- Configuration ---
TEST_IMG_DIR = "dataset/test/images/"
TEST_MASK_DIR = "dataset/test/1st_manual/"
MODEL_PATH = "attention_unet_drive_200epochs_best.pth" # Points to the new Attention U-Net weights
...
def evaluate():
    print(f"Loading Attention U-Net 200-epoch model on: {DEVICE}")
    ...
    with torch.no_grad():
        for idx, (data, targets) in enumerate(test_loader):
            ...
            if idx < 3:
                ...
                ax[2].set_title(f"Attention U-Net Prediction (Dice: {dice:.3f})")
                ax[2].axis("off")
                
                # Save the visual comparison for the first few test samples
                save_path = os.path.join(OUTPUT_DIR, f"result_comparison_attention_unet_200epochs_{idx+1}.png")
                plt.savefig(save_path, bbox_inches='tight')
                plt.close(fig)
                print(f"Saved visual comparison to {save_path}")

    n = len(test_loader)
    print("\n--- Final Attention U-Net Test Set Results ---")
