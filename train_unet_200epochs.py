...
# Import data loader and the standard U-Net model
...
LEARNING_RATE = 1e-4
BATCH_SIZE = 4
NUM_EPOCHS = 200         # Updated to 200
PATIENCE = 30            # Stop if no improvement for 30 epochs
MIN_IMPROVEMENT = 0.001  # Minimum Dice improvement required to reset patience
...
# --- Main Execution ---
def main():
    print(f"Initializing 200-epoch training on: {DEVICE}")
    ...
    best_dice = 0.0
    patience_counter = 0
    best_model_path = "unet_drive_200epochs_best.pth"
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with open(csv_file, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["unet_200epochs", epoch+1, avg_loss, avg_acc, avg_sens, avg_spec, avg_dice])
